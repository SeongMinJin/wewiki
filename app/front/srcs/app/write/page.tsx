...
export default function Write() {
	...
	useEffect(() => {
		...
		async function createNewWiki() {
			try {
				const res = await fetch(`${process.env.NEXT_PUBLIC_API_HOST}:${process.env.NEXT_PUBLIC_API_PORT}/wiki/create/new`, {
					method: "post",
					credentials: "include",
				}).then(res => res.json());
		...
		<div className="relative w-screen min-w-[350px] h-screen flex tablet:grid tablet:grid-cols-2">
			<div className="relative flex flex-col w-full h-full pb-20 phone:pt-8 phone:px-12">
				...
				<div className="absolute bottom-0 left-0 z-10 flex justify-between w-full px-4 py-4 text-lg shadow-2xl h-fit shadow-black font-noto">
					...
					<div className="flex gap-x-4">
						...
						<button className="px-4 py-2 text-white bg-red-300 rounded-md whitespace-nowrap hover:bg-opacity-80" onClick={() => console.log(localStorage.getItem('content'))}>
							출간하기
						</button>
					</div>
				</div>
...

			{
				...
				<div className="relative w-full tablet:block">
					<Graph currentWiki={currentWiki} wikies={wikies} />
				</div> : null
			}
		</div>
	)
